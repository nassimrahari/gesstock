...
import { updatePurchase, fetchPurchasesItem } from '../../services/purchaseService';
...
const PurchaseUpdateComponent = () => {
    ...
    useEffect(() => {
        const fetchItem = async () => {
            const result = await fetchPurchasesItem(itemEditId);
            setId(result.id);

            if(result.purchase_date) {
                setFieldPurchase_date(result.purchase_date.split("T")[0]);
            }

            if(result.supplierId) {
                setFieldSupplier(result.supplierId);
            }

            const newPurchaseLines = result.purchaseLines.map((ligne) => {
                const radom_index = String(Math.random());
                return {
                    ...ligne,
                    uuid: radom_index,
            ...
            setPurchaseLines(newPurchaseLines);
        };
    ...
    return (
        <Dialog open={openUpdate} onOpenChange={setOpenUpdate}>
            ...
            >
                <DialogHeader>
                    <DialogTitle className="text-xl font-bold">Modifier Achat</DialogTitle>
                </DialogHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4 bg-background">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-background rounded-md p-2 flex-grow overflow-y-auto" style={{ maxHeight: "350px" }}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3" bg-background>
...
export default PurchaseUpdateComponent;