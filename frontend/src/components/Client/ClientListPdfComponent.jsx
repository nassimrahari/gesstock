...
const ClientListPdfComponent = ({clients}) => {
    ...
    const clientChunks = chunkArray(clients, 15);
		...
		<div>
			<div ref={contentRef} style={{ display: `${isGenerating ? 'block' : 'none'}` }}>
                ...
                {clientChunks.map((chunk, index) => (
                    <div key={index}>
                        <h1 className='text-center'>Client List (Page {index + 1})</h1>
                        <div className="px-2">
                            <Table>
                                <TableHeader>
			...
			<Button onClick={convertToPdf} disabled={isGenerating}>
				{isGenerating ? 'Generating PDF...' : 'Download  Client List as PDF'}
			</Button>
		</div>
	);
...
export default ClientListPdfComponent;
